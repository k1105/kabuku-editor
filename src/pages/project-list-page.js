 ...
 */
import { iconEl, iconButton } from '../ui/icons.js';
import { createUserBadge } from '../ui/auth-gate.js';
import {
  listFontProjects, createFontProject, renameFontProject, deleteFontProject,
} from '../core/project.js';
import {
  listAnimationProjects, createAnimationProject, renameAnimationProject, deleteAnimationProject,
...
function createFontProjectCard(item, refresh) {
  ...
  main.addEventListener('click', () => { location.hash = `#/font/${item.id}`; });
  ...
  delBtn.addEventListener('click', async (e) => {
    e.stopPropagation();
    if (!confirm(`Typeset「${item.name}」を削除しますか? (関連アニメは残ります)`)) return;
    try {
...
async function renderAnimationColumn(col) {
  ...
  addBtn.addEventListener('click', async () => {
    let typesets;
    try { typesets = await listFontProjects(); }
    catch (e) { alert(`Typeset 一覧の取得に失敗: ${e.message}`); return; }
    if (typesets.length === 0) {
      alert('先に Typeset を作成してください。Animation は Typeset のスナップショットを取り込みます。');
      return;
    }
    ...
    addBtn.disabled = true;
    try {
      const id = await createAnimationProject({ name: name.trim(), fontProjectId: choice.id });
  ...
  for (const item of items) {
    list.appendChild(createAnimationCard(item, () => renderAnimationColumn(col)));
  }
}
...
function createAnimationCard(item, refresh) {
  ...
  main.type = 'button';
  main.className = 'project-card-main';
  main.addEventListener('click', () => { location.hash = `#/animation/${item.id}`; });
...
function pickFontProjectDialog(typesets) {
  return new Promise((resolve) => {
    ...
    for (const ts of typesets) {
      ...
      btn.addEventListener('click', () => { cleanup(); resolve(ts); });
      li.appendChild(btn);
      list.appendChild(li);
    }
